...
def train(args, split, save_path):
    """
    '--base-model', type=str, default='attention',
    ...
    for epoch in range(args.max_epoch):
        ...
        for _, seq, gtscore, cps, n_frames, nfps, picks, _ in train_loader:
            ...
            target_bboxes = bbox_helper.seq2bbox(target) #seq 배열의 1인 원소의 연속된 프레임 번호를 박스로 묶음 
            target_bboxes = bbox_helper.lr2cw(target_bboxes)
            anchors = anchor_helper.get_anchors(target.size, args.anchor_scales)
            # Get class and location label for positive samples # ('--pos-iou-thresh', default=0.6)
            cls_label, loc_label = anchor_helper.get_pos_label(
                anchors, target_bboxes, args.pos_iou_thresh)

            # Get negative samples #('--neg-iou-thresh', default=0.0)
            ...
            # Get incomplete samples #('--incomplete-iou-thresh', default=0.3)
